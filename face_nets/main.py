...
def download_celeba():
    ...
    if not os.path.exists(zip_path):
        ...
        gdown.download(
            id=GOOGLE_DRIVE_ID, output=zip_path, quiet=False
        )
    else:
...
class CelebADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (
            image,
            label[20],
        )  # Example: Use 'Smiling' attribute for classification
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        epoch_acc = val_running_corrects.double() / len(
            val_loader.dataset
        )
...
if __name__ == "__main__":
    ...
    train_dataset = CelebADataset(
        root=root_dir, split="train", transform=transform
    )
    val_dataset = CelebADataset(
        root=root_dir, split="valid", transform=transform
    )
    ...
    print(
        "Model training complete and saved as vit_face_recognition.pth"
    )
